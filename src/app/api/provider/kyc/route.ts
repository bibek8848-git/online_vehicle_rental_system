...
export async function GET(req: NextRequest) {
    ...
    const roleAuth = authorizeRole(user, ['PROVIDER', 'USER']);
    if (!roleAuth.authorized) return roleAuth.errorResponse;

    try {
        await ensureTablesExist();
        const result = await pgPool.query(
            'SELECT id, user_id, document_type, document_url, document_data, extracted_name, extracted_id_number, extracted_dob, extracted_address, extracted_expiry_date, status, rejection_reason, created_at FROM kyc_documents WHERE user_id = $1 ORDER BY created_at DESC',
            [user.id]
        );
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // If auto-approved, set to APPROVED. If pending, set to PENDING unless already APPROVED.
        if (status === 'APPROVED') {
            await pgPool.query(
        ...
        const message = status === 'APPROVED' 
            ? `Your ${documentType} has been automatically verified.` 
            : `Your ${documentType} has been uploaded and is pending manual review. ${rejectionReason || ''}`;
